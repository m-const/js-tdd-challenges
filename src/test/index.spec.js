  ...
  describe("Happy Path", () => {
    ...
    it(`Happy Path: Should return an array of ${expected} members`, () => {
      assert.includeMembers(
        res,
        expected,
        `Should return this ${expected} set of members`
      );
    });
  });
  ...
  describe("Negative Cases", () => {
    ...
    it(`Negative: Should return boolean false if param other than positive integer is provided`, () => {
      assert.strictEqual(
        resNeg,
        false,
        `Should return false if parm is not a number`
      );
    });
    ...
    it(`Negative: Should return boolean false if param is NaN`, () => {
      assert.strictEqual(
        resNaN,
        false,
        `NaN was provided and response was NOT false`
      );
    });
  });
});
...
describe("palindrome() unit tests", () => {
  ...
  describe("Edge Cases", () => {
    ...
    it(`Edge Case True: Should return a bool type`, () => {
      assert.isBoolean(
        resPalindromeTrueSymbols,
        "Edge Case True, palindrome with noise characters (^A-z) returned  other than bool"
      );
    });
    it(`Edge Case True: Should return true because a real palindrom was provided noise characters (^A-z)`, () => {
      assert.isTrue(
        resPalindromeTrueSymbols,
        `A string that is a palindrome with noise characters (^A-z) returned false`
      );
    });
    ...
    it(`Edge Case Null: A string less than 3 characters was provieded`, () => {
      assert.isNull(
        resPalindromeNullNotString,
        `A string that is too short returned not null`
      );
    });
  });
});
